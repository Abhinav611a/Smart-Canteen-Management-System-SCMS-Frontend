    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {loading ? 'Loading…' : `${menu.filter(i => i.available).length} items available`}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {MENU_CATEGORIES.map(cat => (
            ...
            >
              {cat !== 'All' && MENU_CATEGORY_EMOJIS[cat]} {MENU_CATEGORY_LABELS[cat] ?? cat}
            </button>
          ))}
        </div>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <button onClick={() => { setSearch(''); setCategory('All') }} className="text-brand-500 text-sm mt-2 hover:underline">
            Clear filters
          </button>
        </div>
      )}
          ...
          <AnimatePresence>
            {filtered.map((item, i) => (
              <motion.div
                ...
                className={`glass-card p-4 flex flex-col gap-3 group hover:shadow-glow transition-all duration-300 ${!item.available ? 'opacity-60' : ''}`}
              >
                {/* Image area */}
                <div className="relative">
                  ...
                  {!item.available && (
                    <div className="absolute inset-0 rounded-xl bg-gray-900/60 flex items-center justify-center">
                      <span className="text-white text-xs font-bold bg-gray-900/80 px-3 py-1 rounded-full">Unavailable</span>
                    </div>
                  )}
                  <span className="absolute top-2 left-2 badge badge-gray text-[10px]">{MENU_CATEGORY_EMOJIS[item.category]} {MENU_CATEGORY_LABELS[item.category] ?? item.category}</span>
                  {item.ordersToday > 30 && (
                    <span className="absolute top-2 right-2 badge badge-green text-[10px]">🔥 Popular</span>
                  )}
                </div>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <p className="text-brand-600 dark:text-brand-400 font-bold text-sm shrink-0">{formatCurrency(item.price)}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-xs text-gray-400">{item.ordersToday} ordered today</span>
                  </div>
                </div>

                {/* Cart control */}
                {cartQty(item.id) > 0 ? (
                  <div className="flex items-center justify-between bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 rounded-xl px-3 py-2">
